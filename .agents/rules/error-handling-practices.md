...
All C code in the MEFS project must adhere to consistent, robust, and clean error handling.
...
- Always log error messages to `stderr` with clear context (module, line, function):
  ...
  ```
- If using library routines (e.g. CFITSIO, FFTW), print specific library error messages and codes.
...
The return values of file operations, memory allocations/mappings, and system/library calls MUST be inspected:
- `open`, `close`, `read`, `write`, `malloc`, `realloc`, `calloc`, FITS API calls.
- If a failure is detected, log the error and propagate it up the call stack.
